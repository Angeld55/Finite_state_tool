...
SubsequentialTransducer MinTrans(const vector<pair<string, string>>& input, size_t maxWordLength)
{
	...
	SubsequentialTransducer transduser(maxWordLength);
	...
	for (int wordInd = 0; wordInd < input.size(); wordInd++)
	{
		...
		{
			transduser.SET_TRANSITION(i - 1, getChar(previousWord, i), transduser.FindMinimized(i));
		}
		...
		{
			transduser.CLEAR_STATE(i);
			transduser.SET_TRANSITION(i - 1, getChar(currentWord, i), i);
		}

		if (currentWord != previousWord)
		{
			transduser.SET_FINAL(currentWord.size(), true);
			transduser.SET_STATE_OUTPUT(currentWord.size(), {});
		}
		for (size_t j = 1; j <= prefixLengthPlus1 - 1; j++)
		{

			size_t TEMP = transduser.OUTPUT(j - 1, getChar(currentWord, j));
			size_t commonPrefix = min(TEMP, currentOutputNumber);
			size_t wordSuffix = TEMP - commonPrefix;
			transduser.SET_OUTPUT(j - 1, getChar(currentWord, j), commonPrefix);


			transduser.addNumberToAllTransitionsFromState(j, wordSuffix);

			if (transduser.FINAL(j) && wordSuffix != 0)
				transduser.addNumberToOutputState(j, wordSuffix);

			currentOutputNumber -= commonPrefix;
			transduser.handleStateChange(j);
		...
		if (currentWord == previousWord)
		{
			...
			transduser.SET_OUTPUT(prefixLengthPlus1 - 1, getChar(currentWord, prefixLengthPlus1), currentOutputNumber);
	...
	transduser.initialState = transduser.FindMinimized(0);
	transduser.uselessStates = maxWordLength;

	for (int i = 0; i < transduser.uselessStates; i++)
	{	
		transduser.removeFromDict(i);
	}
